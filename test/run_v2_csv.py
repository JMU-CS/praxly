...
URL = "http://localhost:5173/v2/"  # https://praxly.cs.jmu.edu/v2/
...
def main(csv_name, html_name):
    ...
    editor = driver.find_element(By.CLASS_NAME, "cm-content")
    play = driver.find_element(By.ID, "run-button")
    stdout = driver.find_element(By.ID, "stdout")
    stderr = driver.find_element(By.ID, "stderr")
    reset = driver.find_element(By.ID, "reset-button")
    yes = driver.find_element(By.ID, "confirm-reset")
    ...
    for row in csv.reader(file):
        ...
        driver.execute_script(
            "EDITOR_VIEW.dispatch({changes: {from: 0, to: EDITOR_VIEW.state.doc.length, "
            "insert: `" + code + "`}});"
        )
        editor.click()
        play.click()
